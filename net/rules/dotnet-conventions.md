...
- Porta: `{ApplicationProjectDir}/Abstractions/{Aggregate}/I{Aggregate}Repository.cs`
- Impl: `{InfrastructureProjectDir}/Persistence/Repositories/{Aggregate}Repository.cs`
...
public sealed class {Aggregate}Repository(
    IEfCoreRepository<{Aggregate}, {DbContextType}> efRepository) 
      : I{Aggregate}Repository
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await efRepository.DbContext
          .SaveChangesAsync(cancellationToken);   // limite transacional (CONV-081)
    }
...
public sealed class AccountRepository(
    IEfCoreRepository<Account, {DbContextType}> efRepository) 
      : IAccountRepository
{
    // uma única chamada async, nada depois dela → sem async/await (CONV-088); sempre chaves (CONV-089)
    ...
    CancellationToken cancellationToken)
{
    ...
    await efRepository.DbContext
      .SaveChangesAsync(cancellationToken);   // limite transacional, 1 agregado, sem UoW
}
```
